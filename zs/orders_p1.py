...
g0 = {
    ...
    '高密度脂蛋白胆固醇(HDL-ch)', '游离甲状腺素(fT4)', '甲肝病毒抗体IgM(HAV-IgM)',
    ...
    '白血病/淋巴瘤免疫分型初筛(外周血)', '胰岛素注射器(美国BD 328421 1ml 1支/包)', 'B细胞淋巴瘤免疫分型(骨髓)',
    '一次性使用脉搏血氧饱和度探头(MED-LINK ET )', 'I型前胶原氨基端肽（PINP)', '注射用复方维生素(3)', '复方氯化钠注射液',
    '质子泵抑制剂相关基因', '一次性使用静脉留置针(苏州林华 0.6*16 直通型 26G*0.63IN)', '门冬胰岛素注射液', '那格列奈片',
    '一般细菌涂片(格兰染色/找细菌)_痰', '中换药', '精蛋白锌重组赖脯胰岛素混合注射液', '地西泮片',
    '一次性无菌输液接头(爱贝尔 FL-NF01 正压接头)', '成熟红细胞形态观察', '异常白细胞分类', '耐药三通(贝朗 16494C 耐药)',
    '密闭式防针刺伤型静脉留置针(美国BD 383852 Y型 1.3mm*30mm 末端带有1个接头肝素帽、1', '骨髓染色体核型分析(肿瘤)（外送)',
    '层粘连蛋白(LN)', '微量泵输液', '注射用头孢他啶', '双鹤0.9-氯化钠注射液', '注射用兰索拉唑', '血氧饱和度监测',
    '硝酸咪康唑乳膏', '无针输液接头(SMITH SM5000 单头)', '酚麻美敏片', '辰欣0.9%-氯化钠注射液', '骨髓细胞学综合检验',
    '一次性使用静脉留置针(日本泰尔茂 18G B型 三通带肝素帽 18G)', '真菌涂片(找真菌)_痰', 'I型胶原羧基端肽β交联肽(β-cTX)',
    '开塞露', 'Ⅳ型胶原(PⅣP)', '一次性使用静脉留置针(苏州林华 FII-B24G*0.75IN 0.7*19mm)',
    '密闭式防针刺伤型静脉留置针(美国BD 383734 22G Y型)', '甲钴胺片', '银杏叶提取物片', '氯硝西泮片',
    '预充式导管冲洗器(美国BD 5ML 5ML)', 'Ⅲ型胶原(PⅢP)', '兰索拉唑肠溶片', '百特10%-葡萄糖注射液',
    '留置针(苏州林华 ZFII-B 22G×1.00IN（0.9×25mm)50支/盒)', '浓氯化钠注射液', '结核菌涂片(找抗酸杆菌)_痰', 'κ轻链',
    '曲霉培养_痰', '注射用丹参多酚酸盐', '艾司奥美拉唑镁肠溶片', '免疫球蛋白IgE(IgE)',
    '骨髓IGH/CCND1(BCL1) t(11;14)(q13;q32) FISH检测', '一次性使用静脉留置针(日本泰尔茂 18G B型 三通带无针输液接头)',
    '血沉', '结核菌培养_痰', '自粘性软聚硅酮有边型泡沫敷料(瑞典墨尼克 595300 5片/盒 10*10cm)', '羟基脲片',
    '一次性穿刺活检针(Gallini AC -)', '外周血血小板形态观察', '输液器(山东新华 JMQ(PZG1) 精密过滤(带针)PVC)',
    '"无针输液接头(贝朗 4097154N Safeflow 无针输液接头', '网织红细胞计数', '骨髓P53 17p13.1 FISH检测', '兰索拉唑胶囊',
    '自粘性外科敷料(瑞典墨尼克 670800 6*7cm 60片/盒 480片/箱)', '留置针(苏州林华 ZFII-B 26G×0.63IN（0.6×16mm) 50支/盒)',
    '多磺酸粘多糖乳膏', '天瑞5%-葡萄糖注射液', '留置针(苏州林华 0.6*19 三通型 26G*0.75IN)',
    '留置针(日本泰尔茂 22G 带肝素帽 22G 带肝素帽)', '透明质酸(HA)', '氯吡格雷药物基因CYP2C19', '格列喹酮片',
    '一次性使用呼吸过滤器包(斯莱达 EM12-601A 呼吸管路型)', '密闭式防针刺伤型静脉留置针(美国BD 383714 24G Y型)',
    '30R笔-精蛋白人胰岛素注射液', '同型半胱氨酸(HCY)', '泮托拉唑钠肠溶胶囊', '百特5%-葡萄糖注射液',
    '注射器(美国BD 306594 不带针 5ML)', '一次性穿刺活检针(Gallini BM -)', '西洛他唑片', '小换药'
}
g1 = {
...
g3 = {
    ...
    '盐酸曲美他嗪缓释片', '辅酶Q10注射液', '盐酸曲美他嗪胶囊', '盐酸多巴酚丁胺注射液', '注射用二丁酰环磷腺苷钙',
...
g_ac = {
    '盐酸贝那普利片', '培哚普利叔丁胺片', '培哚普利氨氯地平片（Ⅲ)', '培哚普利片', '雷米普利片',
    '卡托普利片',
}
g_ar = {
    '缬沙坦胶囊', '氯沙坦钾片', '厄贝沙坦片', '奥美沙坦酯片', '沙库巴曲缬沙坦钠片',
    '奥美沙坦酯氨氯地平片', '氯沙坦钾氢氯噻嗪片', '厄贝沙坦氢氯噻嗪片', '替米沙坦片',
    '坎地沙坦酯片', '缬沙坦氨氯地平片',
}
g_cc = {
    '苯磺酸左旋氨氯地平片', '硝苯地平控释片', '注射用硝普钠', '苯磺酸氨氯地平片', '培哚普利氨氯地平片（Ⅲ)',
    '奥美沙坦酯氨氯地平片', '非洛地平缓释片', '硝苯地平片', '缬沙坦氨氯地平片', '苯磺酸左氨氯地平片',
}
g_di = {
    '螺内酯片', '托拉塞米注射液', '托拉塞米胶囊', '呋塞米注射液', '注射用呋塞米',
    '注射用托拉塞米', '托拉塞米片', '呋塞米片', '氯沙坦钾氢氯噻嗪片', '厄贝沙坦氢氯噻嗪片',
}
g_st = {
